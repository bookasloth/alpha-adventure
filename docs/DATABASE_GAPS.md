...
| Free-text statuses | `profiles.status`, `profiles.medical_clearance_status` text; `leads.status` text (enum `lead_status` unused) | inconsistent values | apply enums / check constraints |
...
- **Reproducibility:** a fresh project run through 0001→0005 **should** produce
  ...
  - Migrations were applied to the live DB by a separate session; **live == files
    is NOT VERIFIED** (reconcile via the introspection SQL).
